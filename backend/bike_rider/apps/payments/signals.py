...
def update_user_balance(sender, instance):
    sum_payments = Payment.objects.filter(user=instance.user, confirmed=True).aggregate(
        Sum('total_eur_cent'))['total_eur_cent__sum']
    sum_charges = Charge.objects.filter(user=instance.user).aggregate(
        Sum('total_eur_cent'))['total_eur_cent__sum']
